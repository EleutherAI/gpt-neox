...
def get_init_methods(**kwargs):
    def _get(name):
        if name == "normal":
            return init_method_normal(kwargs.get("init_method_std"))
        elif name == "scaled_normal":
            return scaled_init_method_normal(
                kwargs.get("init_method_std"), kwargs.get("num_layers")
            )
        elif name == "orthogonal":
            return orthogonal_init_method()
        elif name == "scaled_orthogonal":
            return orthogonal_init_method(kwargs.get("num_layers"))
        elif name == "xavier_uniform":
            return xavier_uniform_init_method()
        elif name == "xavier_normal":
            return xavier_normal_init_method()
        elif name == "wang_init":
            return wang_init_method(kwargs.get("num_layers"), kwargs.get("hidden_size"))
        elif name == "small_init":
            return small_init_init_method(kwargs.get("hidden_size"))
        else:
            raise NotImplementedError(f"Unkown init method {name}")

    return _get(kwargs.get("init_method")), _get(kwargs.get("output_layer_init_method"))
