...
* A python dataclass provides attributes (similar in use to an argparse Namespace)
* The main class 'NeoXArgs' (in ./arguments) needs to be instantiated; it exposes all configuration attributes that are relevant to GPT NeoX
...
* it is possible to set undefined attributes (e.g. line of code 'NeoXArgs().my_undefined_config = 42' works fine); such set attributes are not validadet
...
* In order to avoid setting undefined attributes use the function NeoXArgs().update_value(); this function raises an error if the to be set attribute is not defined
...
* NeoX args (in ./arguments) inherits from a the following subclasses: NeoXArgsDeepspeedRunner, NeoXArgsDeepspeedConfig, NeoXArgsModel, NeoXArgsTokenizer, NeoXArgsTraining, NeoXArgsParallelism, NeoXArgsLogging, NeoXArgsOther, NeoXArgsTextgen
* Subclasses somewhat sort attributes by purpose
* The attributes of NeoXArgsDeepspeedRunner are directly mapped to the expected command line args of deepspeed.launcher.runner.main; no attributes unknown to deepspeed should be included; no arguments relevant for deepspeed should be ommmitted
* The attributes of NeoXArgsDeepspeedConfig are directly mapped to the expected keys of the deepspeed config; no arguments relevant for deepspeed should be ommmitted
