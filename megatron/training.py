...
def get_learning_rate_scheduler(optimizer, neox_args):
        ...
        if neox_args.override_lr_scheduler:
                return None
        ...
        init_step = neox_args.iteration if neox_args.iteration else 0
        warmup_iter = neox_args.warmup * num_iters
        print_rank_0(f"SENTINEL '{init_step}'")
...
def setup_model_and_optimizer(neox_args, use_cache=False, iteration=None):
        ...
        if neox_args.deepspeed:
                ...
                if not neox_args.override_lr_scheduler:
