...
class Adapter(nn.Module):
    def __init__(self, neox_args, activation=nn.GELU):
        super().__init__()
        self.adapter_config = neox_args.adapter_config
        self.add_norm = neox_args.adapter_config.get("add_norm", False)
        if self.add_norm:
            self.norm = get_norm(neox_args)
        self.init_method, _ = get_init_methods(neox_args)
        self.downsample_factor = neox_args.adapter_config.get("downsample_factor", 4)
        self.in_proj = mpu.ColumnParallelLinear(
            neox_args=neox_args,
            input_size=neox_args.hidden_size,
            output_size=neox_args.hidden_size // self.downsample_factor,
            gather_output=False,
        ...
        self.out_proj = mpu.RowParallelLinear(
            neox_args=neox_args,
            input_size=neox_args.hidden_size // self.downsample_factor,
...
class ParallelMLP(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.add_adapter:
            output = self.adapter(output + output_bias)
            return output, None
...
class ParallelTransformerLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, layer_past=None):
        ...
        attention_output, attention_bias = self.attention(
            layernorm_output, attention_mask, layer_past=layer_past
        )
