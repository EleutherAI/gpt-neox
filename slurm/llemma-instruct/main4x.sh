...
#SBATCH --array=0
#SBATCH --output=muinstruct4x_%a_%A.out
#SBATCH --error=muinstruct4x_%a_%A.out
...
run_name="llemma_7b_muinstruct4x_10epoch_${lr}lr_v0.2"
...
HF_DIR="${SAVE_DIR}-hf"
python tools/convert_llama_sequential_to_hf.py --input_dir ${SAVE_DIR}/global_step399 --config_file ${CONF_PATH} --output_dir $HF_DIR && cp -v /nobackup/scratch/usr/za2514/codellama/tokenizer.model $HF_DIR
