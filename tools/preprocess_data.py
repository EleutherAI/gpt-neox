...
def yield_from_files(fnames: list, semaphore, jsonl_keys):
    """
    ...
    def yielder(fname, semaphore):
        for f in filter(lambda x: x, lmd.Reader(fname).stream_data(jsonl_key=jsonl_keys[0])):
            semaphore.acquire()
            yield f
...
def main():
    ...
    fin = yield_from_files(args.input.split(","), semaphore, jsonl_keys=args.jsonl_keys)
