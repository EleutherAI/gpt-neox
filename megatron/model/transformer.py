...
class ParallelSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.attention_type != "multihead":
            self.num_kv_heads_per_partition = mpu.divide(neox_args.hidden_size, neox_args.num_kv_heads) # TODO: we want to clone single-kv heads across ranks...
            self.kv_hidden_size = (neox_args.num_attention_heads // neox_args.num_kv_heads) *  self.hidden_size_per_attention_head
        ...
        if self.attention_type == "multihead":
            ...
            self.query_key_value = mpu.ColumnParallelLinear(
                neox_args=neox_args,
                input_size=neox_args.hidden_size,
                output_size=neox_args.hidden_size + 2 * kv_hidden_size,
                gather_output=False,
                init_method=init_method,
                bias=neox_args.use_bias_in_attn_linear,
    ...
    def flash_attention(self, query_layer, key_layer, value_layer):
        ...
        # [nq/nkv]
        nkv_per_partition = self.num_kv_heads_per_partition

        if self.pos_emb != "alibi":

            # [sk, b, nq/nkv, hn] -> [b, sk, nq/nkv, hn] 
            key_layer = key_layer.transpose(0, 1).reshape(
                output_size[0], output_size[3], nkv_per_partition, -1
            )
    ...
    def forward(self, hidden_states, attention_mask, layer_past=None):
        ...
        if self.attention_type == "multihead":
            ...
            (query_layer, key_layer, value_layer) = [x.contiguous() for x in torch.split(mixed_x_layer, [self.num_attention_heads_per_partition * self.hidden_size_per_attention_head, self.num_kv_heads_per_partition * self.hidden_size_per_attention_head, self.num_kv_heads_per_partition * self.hidden_size_per_attention_head], dim=mixed_x_layer.dim() - 1)]
