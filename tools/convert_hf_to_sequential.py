...
/gpfs/alpine/csc499/scratch/kshitijkg
...
CUDA_VISIBLE_DEVICES=0 python tools/convert_hf_to_sequential.py \
    --size 70m \
    --revision 143000 \
    --output-dir /gpfs/alpine/csc499/scratch/kshitijkg/neox_converted/pythia/70m \
...
MULTI_GPU_ARGS = " ".join(
        [
            "--hf-model-name pythia-410m-v0",
            "--revision 143000",
            "--output-dir /gpfs/alpine/csc499/scratch/kshitijkg/neox_converted_magma/mp1_pp1/pythia/410m",
            "--cache-dir /gpfs/alpine/csc499/proj-shared/hf_checkpoints",
            "--config configs/llama/410M.yml configs/local_setup_llama.yml",
            "--test"
        ]
    ...
    for layer_hf in range(num_layers):
        ...
        for key in hf_model.gpt_neox.layers[0].state_dict().keys():
            ...
            if "attention." in key:
                ...
                seq_state_dict[f"sequential.{layer_seq}.{new_key}"] = hf_layer_sd[key]
            elif "mlp." in key:
...
def get_non_existing_dir(tmp_dir):
    while os.path.exists(tmp_dir):
        tmp_dir = os.path.join(tmp_dir, "tmp_dir_"+ str(np.random.randint(10000)))
    return tmp_dir
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ckpt-tmp-dir",
        default="/gpfs/alpine/csc499/scratch/kshitijkg/ckpt-tmp",
        help="Directory to store cached hugging face checkpoints."
    )
    ...
    print("Warning the following script will delete this directory withing {}".format(tmp_cache_dir))
    print("======================================================================")
    time.sleep(5)
