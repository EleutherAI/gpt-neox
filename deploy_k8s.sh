...
do
    ...
    echo "chmod 600 ~/.ssh/authorized_keys && chmod 700 ~/.ssh && chown -R root /root/.ssh && git reset --hard && git checkout $BRANCH && git pull origin $BRANCH && pip uninstall -y deepspeed && pip install git+git://github.com/EleutherAI/DeeperSpeed@master" | kubectl exec --stdin $id -- /bin/bash
done
