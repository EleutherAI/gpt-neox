...
def initialize_neox(neox_args, allow_no_cuda=False):
    ...
    This function does the following:
        ...
        3. Loads the fused kernels
        4. Sets the model parallel rank and world size
        5. if neox_args.lazy_mpu_init == True, returns a function to finalize distributed initialization.

    Args:
        neox_args: NeoX arguments
        allow_no_cuda: If True, does not raise an error if no cuda device is found.

    Returns:
        (Optionally) A function to finalize distributed initialization.
    """
    ...
    def finish_mpu_init():
        # Pytorch distributed, 3D topology, mpu and activation checkpointing initialization.
...
def _initialize_distributed(neox_args):
    """
    This function does the following:
        1. Launches torch.distributed.
        2. Sets up the 3D topology (model / pipe / data parallelism).
        3. Initializes the mpu (Model Parallel Utilities).
        4. Initializes Activation checkpointing.
    """
