...
def convert_model(output_base_path, input_base_path, model_size: str, num_output_shards: int):
    ...
    for i in range(num_input_shards):
        ...
        pbar.update(1)
    helper = Helper(loaded=loaded, model_path=model_path, num_output_shards=num_output_shards, model_size=model_size)
    ...
    for layer_i in range(num_layers):
        ...
        for out_rank in range(num_output_shards):
            ...
            }, helper.save_path(layer_i=layer_i + 2, rank=out_rank))
...
class Helper:
    def __init__(self, loaded, model_size, num_output_shards, model_path):
        self.loaded = loaded
    ...
    def slice_rank(self, x, rank: int, dim: int, clone=True):
        assert self.num_output_shards == x.shape[dim]
        # noinspection PyTypeChecker
        out = x[slice(*(([None] * dim) + [rank]))]
        if clone:
            out = out.clone()
        return out
