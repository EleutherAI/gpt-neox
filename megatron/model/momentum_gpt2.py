...
class GPT2Momentum(torch.nn.Module):
    ...
    def __init__(self, neox_args, num_tokentypes=0, parallel_output=True, topology=None, inference=False,
        ...
        self.n_iters = 3  # number of momentum iterations - hardcode to 12 for now
        ...
        if self.neox_args.checkpoint_activations:
            raise NotImplementedError # momentumnet needs its own custom grad checkpointing - not yet implemented
    ...
    def forward(self, x, ts=0, vs=1):
        ...
        for _ in range(self.n_iters):
            for layer in self.transformer_layers:
                x = layer(x)
                hidden = x[0]
                v = self.gamma * v + hidden * ts * (1 - self.gamma)
                hidden = (hidden * vs) + (v * ts)
                x = (hidden, *x[1:])
        if self._inference:
