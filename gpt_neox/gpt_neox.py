...
class GPTNeoX_Pipe(PipelineModule):
    def __init__(self, *, num_tokens, dim, seq_len, depth, loss_fn, heads = 8, 
        ...
        for i in range(depth):
            spec.append(LayerSpec(TransformerBlock, dim = dim, seq_len = seq_len, heads = heads, 
	                    dim_head = dim_head, attn_dropout = attn_dropout, ff_dropout = ff_dropout, 
	                    sparse_attn = layers_sparse_attn[i], norm_class = norm_class))
        
        #Output norm and Linear
        spec += [LayerSpec(norm_class, dim),
