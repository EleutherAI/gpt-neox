...
def build_train_valid_test_data_iterators(neox_args):
    from lm_dataloader import LMDataset, BlendableDataset, make_indexed_dataset
    ...
    has_dataloader = mpu.get_model_parallel_rank() == 0 and pipe_load
    if has_dataloader:
        ...
        if neox_args.train_data_paths:
            # Build datasets + blend if multiple data_paths are provided.
            train_datasets, valid_datasets, test_datasets = [], [], []
            ...
            # Blend datasets.
            if train_datasets:
                train_ds = BlendableDataset(
                    train_datasets,
                    weights=neox_args.train_data_weights,
                    weight_by_num_docs=neox_args.weight_by_num_documents,
                    weighted_sampler_alpha=neox_args.weighted_sampler_alpha,
                )
            if valid_datasets:
                valid_ds = BlendableDataset(
                    train_datasets,
                    weights=neox_args.valid_data_weights,
                    weight_by_num_docs=neox_args.weight_by_num_documents,
                    weighted_sampler_alpha=neox_args.weighted_sampler_alpha,
                )
            if test_datasets:
                test_ds = BlendableDataset(
                    train_datasets,
                    weights=neox_args.test_data_weights,
                    weight_by_num_docs=neox_args.weight_by_num_documents,
                    weighted_sampler_alpha=neox_args.weighted_sampler_alpha,
                )
        else:
            ...
            dataset = make_indexed_dataset(
                neox_args.data_path, skip_warmup=not neox_args.mmap_warmup
            )
            from megatron.utils import ddb

            ddb()
            train_ds, valid_ds, test_ds = dataset.split(
                splits=neox_args.split,
                num_samples=train_val_test_num_samples,
                seq_length=neox_args.seq_length,
                mode=getattr(neox_args, "dataset_mode", "pack"),
