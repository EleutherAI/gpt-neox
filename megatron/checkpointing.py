...
def load_checkpoint(
	...
	if neox_args.deepspeed:
		...
		load_scheduler = (
			not neox_args.override_lr_scheduler
		)
		if not load_scheduler:
				assert not neox_args.use_checkpoint_lr_scheduler, \
				"`use_checkpoint_lr_scheduler` set to True but `override_lr_scheduler` is also True. \
				`override_lr_scheduler` should be used to control whether to load scheduler from ckpt."
		...
		checkpoint_name, state_dict = model.load_checkpoint(
			...
			load_lr_scheduler_states=load_scheduler,
			load_module_only=not (load_optim or load_scheduler),
			tag=tag,
		)
