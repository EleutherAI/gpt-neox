...
class AliBi(torch.nn.Module):
    def __init__(self, num_heads, mp_size=1, mp_rank=1):
        ...
        a = -torch.tril(
            torch.arange(2048).view(2048, 1).repeat(1, 2048)
            + torch.arange(0, -2048, -1)
        ).half()
        slopes = self.slopes.half()
        a = a * slopes.view(slopes.shape[0], 1, 1)
        self.cached_seq_len = 2048
        self.cached_matrix = a.half()
    ...
    def forward(self, x):
        ...
        if seq_len_q != seq_len_k:
            ...
            a = self.cached_matrix[:, seq_len_k - 1, :seq_len_q].view(
                self.cached_matrix.shape[0], 1, seq_len_q
            )  # seq_len_k - 1 points to the last token index in the current inference batch.
        else:
            a = self.cached_matrix[:, :seq_len_k, :seq_len_q]

        return x.add_(a.cuda())
