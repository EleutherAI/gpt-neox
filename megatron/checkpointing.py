...
def load_checkpoint(neox_args, model, optimizer, lr_scheduler, inference=False):
    ...
    if neox_args.deepspeed:
        ...
        load_module_strict = not (soft_prompt_enabled or adapter_enabled)
        # TODO: currently for this to work, changes to deepspeed are required.
        # you need to change this line: https://github.com/EleutherAI/DeeperSpeed/blob/main/deepspeed/runtime/fp16/fused_optimizer.py#L460
        # and this line: https://github.com/EleutherAI/DeeperSpeed/blob/main/deepspeed/runtime/fp16/unfused_optimizer.py#L351
        # to call self.refresh_fp32_params() instead of copying the fp32 params from the state dict.
        if not load_module_strict:
            load_optim_and_scheduler = False
            neox_args.no_load_optim = True
            neox_args.finetune = True
            print_rank_0(
                "Cannot load optimizer and scheduler when finetuning with soft prompts / adapters"
            )
            print_rank_0(
                "Disabling optimizer and scheduler loading & setting finetune to True"
            )
    ...
    if neox_args.finetune:
        ...
        if iteration is None:
            raise ValueError(
                "Unable to load iteration from checkpoint {}, exiting".format(
                    checkpoint_name
                )
            )
