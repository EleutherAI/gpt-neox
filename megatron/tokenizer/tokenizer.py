...
def build_tokenizer(args):
    ...
    if args.tokenizer_type.lower() == "GPT2BPETokenizer".lower():
        ...
        assert args.vocab_file is not None
        assert args.vocab_file.endswith(".model"), (
            "SPMTokenizer only works with model files created by google/sentencepiece. "
            "To use it, make sure that the `vocab_file` isn't from other frameworks "
            "(e.g. tokenizer.json of huggingface/tokenizers). "
        )
